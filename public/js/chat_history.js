...
const getClientList = async ()=>{
  try{
    let Lis = '';
    const clients = await fetch('/clientList', {
        method: 'POST',
        headers: {
...
chat_input.addEventListener('input',async ()=>{
  ...
  const chatUserId    = localStorage.getItem('selectedClientId');
  if (chat_input.value != '') {
    const response = await fetch('/typingEvent', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ currentUserId:currentUserId,chatUserId:chatUserId }),
    });
    if (response.ok) {
      console.log("success");
    }
  }else{
    const response = await fetch('/stopTyping', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
socket.on('typing', (data) => {
  const currentUserId = localStorage.getItem('userId');
  if (currentUserId == data.chatUserId) {
    const receivedDivs = document.querySelectorAll('.recieved-msg-wrapper');
    let found = false;
    receivedDivs.forEach(Element=>{
      if (Element.id == 'dot-loader') {
